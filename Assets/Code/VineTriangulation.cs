...
{
    /// <summary>
    /// Given a spline, generate a mesh around that spline to represent a vine.
    /// Depends on Circle3d to generate the slice polygon.
    /// </summary>
    public class VineTriangulation
    {
        ...
        private void addStack(float l, float Radius)
        {
            ...
            {
                v[i] += p;
                v[i] = RotatePointAroundPivot(v[i], p,
                    Quaternion.FromToRotation(Vector3.up, Spline.EvaluateTangent(l)));
            }
